...
if (isset($_POST['inform'])) {
    $orderDate = date('d-m-Y, H:i:s', strtotime(pg_escape_string($con, $_POST['date'])));
    $phone = pg_escape_string($con, $_POST['phone']);
    $receiveDate = date('d-m-Y', strtotime($orderDate . ' + 5 days'));
    ...
    $postData = array(
        ...
        'encoding' => 0,
        'schedule_time' => '',
        'message' => 'Ndugu ' . $name . ' tunashukuru sana kufanya biashara nasi,
Oda yako ya tarehe ' . $orderDate . ' tumeipokea. Utapata mzigo wako tarehe ' . $receiveDate . '.
Endelea kutumia Ashopie shop. Karibu sana',
        ...
        <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover">
                ...
                <tbody>
                    <tr>
                        <?php
                        $sql = pg_query($con, "SELECT concat(firstname,' ',sirname) as name, users.phone_number, order_date, address, ordered_quantity, order_id
                                                FROM orders, users
                                                WHERE orders.user_id = users.id
                                                AND status_id = 2
                                                GROUP BY order_id,users.phone_number, order_date, address, ordered_quantity, order_id, name
                                                ");
